...
class Solver():
    ...
    def diff_equ(self, time):
        ...
        D_op = 1/2 * np.einsum('Nxos, Nox -> Nsx', D, self.x_der(rho), optimize = True)
            
        V_rho = np.einsum('Nxl, Nlx -> Nlx', V, rho, optimize = True)
        
        rho_next =  rho + self.int_t * (self.x_der(D_op) - self.x_der(V_rho))

...
class Solver():
    ...
    def __init__(self, l = None, x = None, t = None, rho0 = None, c = None,
                 boundary = None, diff = False):
    ...
    def diff_fixed_point_func(self, rho, rho_next, V):
        
        # D, V dimensions N, x, momenta
        
        diff = rho_next
        
        #D_op = self.int_t/2 * np.einsum('Nxos, Nox -> Nsx', D, self.x_der(rho_next),
                                        #optimize = True)       
        
        V_rho = np.einsum('Nxl, Nlx -> Nlx', V, rho_next, optimize = True)
        
        foo = -self.int_t * self.x_der(V_rho) + rho
    ...
    def diff_equ(self, time):
        ...
        for i in range(30):            
            ...
            rho_next, difference = self.diff_fixed_point_func(rho, rho_next, V)
